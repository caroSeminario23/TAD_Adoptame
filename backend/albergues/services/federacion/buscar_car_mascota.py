...
@federado_routes.route("/federado/caracteristica/buscar", methods=["POST"])
def route_buscar_car_mascota():
    ...
    if not resultados:
        return make_response(jsonify({"message": "Característica de mascota no encontrada.", "status": 404}), 404)
    else:
        return make_response(jsonify({"message": "Característica de mascota encontrada.", "status": 200, "data": resultados}), 200)